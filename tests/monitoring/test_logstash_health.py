...
class TestLogstash(unittest.TestCase):
    ...
    def test_logstash_pods_status(self):
        for pod_name in self.logstash_pods:
            command = (
                f"kubectl get pods -n {self.namespace} -o jsonpath='{{range .items[?(@.metadata.name==\"{pod_name}\")]}}"
                f"{{.metadata.name}}{{\" - \"}}{{.status.phase}}{{\"\\n\"}}{{end}}'"
            )
            status = self.k8s_utils.run_command(command).strip()
    ...
    def test_logstash_container_status(self):
        for pod_name in self.logstash_pods:
            command = (
                f"kubectl get pod {pod_name} -n {self.namespace} -o jsonpath='{{range .status.containerStatuses[?(@.name==\"{self.container_name}\")]}}"
                f"{{.name}}{{\" - \"}}{{.state}}{{\"\\n\"}}{{end}}'"
            )
            state = self.k8s_utils.run_command(command).strip()
    ...
    def test_customer_pipeline_exists(self):
        for pod_name in self.logstash_pods:
            command = f"kubectl exec -it {pod_name} -n {self.namespace} -- curl -s http://localhost:9600/_node/pipelines?pretty | grep customer"
            pipeline_data = self.k8s_utils.run_command(command).strip()
    ...
    def test_customer_pipeline_status(self):
        for pod_name in self.logstash_pods:
            command = f"kubectl exec -it {pod_name} -n {self.namespace} -- curl -s http://localhost:9600/_node/stats/pipelines/customer?pretty | grep '\"status\"'"
            status = self.k8s_utils.run_command(command).strip()
    ...
    def test_plugins_existence(self):
        for pod_name in self.logstash_pods:
            ...
            plugin_data = self.k8s_utils.run_command(command).strip()
