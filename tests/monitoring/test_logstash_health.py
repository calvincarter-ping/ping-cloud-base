...
class TestLogstash(unittest.TestCase):
    ...
    def execute_command(self, command, pod_name):
        return self.k8s_utils.exec_command(pod_name, self.namespace, command).strip()
    ...
    def test_logstash_pods_status(self):
        for pod_name in self.logstash_pods:
            ...
            status = self.execute_command(command, pod_name)
    ...
    def test_logstash_container_status(self):
        for pod_name in self.logstash_pods:
            ...
            state = self.execute_command(command, pod_name)
    ...
    def test_customer_pipeline_exists(self):
        for pod_name in self.logstash_pods:
            ...
            pipeline_data = self.execute_command(command, pod_name)
    ...
    def test_plugins_existence(self):
        for pod_name in self.logstash_pods:
            ...
            plugin_data = self.execute_command(command, pod_name)
