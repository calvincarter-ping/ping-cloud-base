...
testOpensearchBootstrapImageTag() {
  ...
  local osBootstrapImage=$(kubectl get pod -n elastic-stack-logging logstash-elastic-0 -o jsonpath='{.spec.initContainers[?(@.name=="opensearch-bootstrap")].image}' | awk -F: '{print $2}')
